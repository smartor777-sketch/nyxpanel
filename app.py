...
SHARED_MODES = {
    "https":           {"http_port": 2501, "service": "mtproxy-shared-https"},
    "https-lanes":     {"http_port": 2502, "service": "mtproxy-shared-https-lanes"},
    "websocket":       {"http_port": 2503, "service": "mtproxy-shared-websocket"},
    "websocket-lanes": {"http_port": 2504, "service": "mtproxy-shared-websocket-lanes"},
}
...
def tproxy_get_shared_backend(carrier_mode):
    """Get the backend address for a shared instance by carrier mode."""
    mode_info = SHARED_MODES.get(carrier_mode)
    if not mode_info:
...
def tproxy_add():
    ...
    mode = request.form.get("mode", "isolated").strip()
...
@app.route("/self/tproxy/<name>/toggle-isolation", methods=["POST"])
def tproxy_toggle_isolation(name):
    """Switch profile between isolated and shared mode."""
    if not is_admin():
        return redirect("/self/login")
    profiles = tproxy_read_profiles()
    target = next((p for p in profiles if p["name"] == name), None)
