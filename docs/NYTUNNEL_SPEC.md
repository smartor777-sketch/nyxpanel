...
Плюс probe resistance (незнакомый запрос → скучный 404) и многоузловость
с арбитром, подписывающим список узлов (Ed25519).
...
Клиент ──HTTPS(TLS 1.3, h2, uTLS)──► NYX-Tunnel сервер ──► целевой хост:порт
          XChaCha20-Poly1305 фреймы внутри          (или UDP-relay, этап 4)
```
