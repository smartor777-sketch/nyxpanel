...
| `/info` | Инструкция для пользователя | Выбор через кнопки |
| `/delete` | Удалить пользователя | Выбор через кнопки |
| `/deactivate` | Деактивировать (временно) | Выбор через кнопки |
| `/activate` | Активировать обратно | Выбор через кнопки |
| `/stats` | Общая статистика | — |
| `/broadcast <текст>` | Рассылка всем пользователям | Текст |
| `/lock` | Включить/выключить регистрации | — (toggle) |

### Лимиты

| Параметр | Значение | Описание |
|----------|----------|----------|
| `MAX_USERS` | 20 | Максимум пользователей |
| `REG_OPEN` | `true` / `false` | Флаг открытия регистраций |

Хранятся в `passwords.json`:
```json
{
  "settings": {
    "max_users": 20,
...
Bot: 🤖 WDTT VPN Manager
      ...
      🔒 Безопасность:
      /lock — 🔓 Регистрации открыты (нажми чтобы закрыть)
...
Bot: 🔒 Регистрации закрыты
      Новых пользователей: 0/20

      [🔓 Открыть регистрации]

Admin: нажимает [🔓 Открыть регистрации]

Bot: 🔓 Регистрации открыты
      Текущих пользователей: 12/20

      [🔒 Закрыть регистрации]
```

### Поведение при закрытых регистрациях

Если `reg_open == false`:
- `/start` (user) → "👋 Привет! Регистрации временно закрыты. Следите за обновлениями."
- `/guide` (user) → Показывает инструкцию, но добавляет предупреждение
- `/help` (user) → Работает нормально
...
### Если регистрации закрыты

```
Admin: /new

Bot: 🔒 Регистрации временно закрыты.
      Максимум: 20 пользователей
      Текущих: 20

      Используйте /lock чтобы открыть регистрации.
```
...
Bot: ❌ Достигнут лимит пользователей (20/20).
      Удалите ненужных через /list
      или увеличьте лимит в settings.
```

---

## 6. Список пользователей (`/list`) с пагинацией

**На странице:** 5 пользователей
...
Bot: 👥 Пользователи (12) — стр. 1/3
      ─────────────────────────

      🟢 AbCdEf1234567890
         ⏰ 28 дн | 📱 Привязан | 📊 1.2 GB
      ...
      🔵 AsDfGh123456789
         ⏰ 90 дн | 📱 Привязан | 📊 45.7 MB

      🟢 ZxCvBn123456789
         ⏰ 15 дн | 📱 Привязан | 📊 3.8 GB
...
```
Bot: 👥 Пользователи (12) — стр. 2/3
      ─────────────────────────

      🟢 HjKlZx123456789
         ⏰ 60 дн | 📱 Привязан | 📊 22.1 GB
...
### Последняя страница (некнопки неактивны)

```
Bot: 👥 Пользователи (12) — стр. 3/3
      ─────────────────────────

      ...

      ─────────────────────────
      [◀️ Назад] [3/3] [Вперёд ▶️]  ← кнопки серые
      [➕ Новый] [📊 Статистика]
```

### Inline-кнопки пользователя (после нажатия на пароль)

```
Admin: нажимает [AbCdEf****]

Bot: 🔍 AbCdEf1234567890
      ─────────────────
      ⏰ Действует: 28 дн (до 01.10.2026)
      🔗 Хэш: XXXXXXXXX
      📱 Устройство: ca1917a0...
      📊 Трафик: ⬇️ 1.2 GB | ⬆️ 0.3 GB
      ─────────────────
      [📱 Инструкция] [⏸ Деактивировать]
      [📱 Отвязать]   [🗑 Удалить]
      [◀️ Назад к списку]
```
...
|--------|---------------|----------|
| ◀️ Назад | `list_prev` | Предыдущая страница |
| Вперёд ▶️ | `list_next` | Следующая страница |
| [AbCdEf****] | `user_AbCdEf1234567890` | Карточка пользователя |
| 📱 Инструкция | `info_AbCdEf1234567890` | Инструкция для юзера |
| ⏸ Деактивировать | `deact_AbCdEf1234567890` | Деактивация |
| ✅ Активировать | `react_AbCdEf1234567890` | Активация |
| 📱 Отвязать | `unbind_AbCdEf1234567890` | Отвязать устройство |
| 🗑 Удалить | `del_AbCdEf1234567890` | Удаление (с подтверждением) |
| ◀️ Назад к списку | `list_back` | Возврат к списку |
| ➕ Новый | `new_user` | Начать создание |
| 📊 Статистика | `show_stats` | Показать статистику |
...
- [ ] Роли (admin/user) и проверка ID
- [ ] Диалог /new (срок → хэш → пароль)
- [ ] Пагинация списка /list (5 на страницу)
- [ ] Inline-кнопки для карточки пользователя
- [ ] Инструкция /info с wdtt:// ссылкой
- [ ] Удаление /delete с подтверждением
- [ ] Деактивация /activate, /deactivate
- [ ] Кнопка /lock (открытие/закрытие регистраций)
- [ ] Лимит пользователей (MAX_USERS=20)
- [ ] Статистика /stats
- [ ] Рассылка /broadcast
- [ ] Systemd сервис
- [ ] Деплой на prod
