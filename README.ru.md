...
| **Telegram MTProxy** | Прокси | Клиенты Telegram |

## Telegram MTProxy

Каждый пользователь получает собственный профиль MTProxy с уникальным секретом. Два режима:

### Изолированный режим (Isolated)
- Выделенный процесс `mtproto-proxy` для каждого пользователя
- Уникальный порт (2401-2499)
- Полное выделение ресурсов (~20-35 MB RAM на инстанс)

### Общий режим (Shared)
- Несколько пользователей делят один процесс `mtproto-proxy`
- Общие порты (2501-2504)
- Экономия RAM — ~20 MB на общий инстанс независимо от числа пользователей
- Каждый пользователь сохраняет свой уникальный секрет
- Shared-инстанс принимает несколько секретов через пул-файл (флаги `-S`)

### Переключение режимов
- Тогл isolated ↔ shared из UI панели
- **Секрет НЕ меняется** — пользователю не нужно обновлять клиент
- Кратковременный restart shared MTProxy (~1с) при изменении пула

### Транспортные режимы

| Режим | HTTP порт | TCP порт |
|-------|-----------|----------|
| HTTPS | 2501 | 3501 |
...
```
/etc/tproxy-server/
├── profiles.json         # Профили MTProxy (уникальные секреты)
├── modes.json            # shared/isolated для каждого профиля
└── tg_mappings.json      # Маппинг пользователей Telegram

/etc/mtproxy/
├── mtproxy-shared-*.secrets   # Пулы секретов для shared-инстансов
└── mtproxy-<name>.env         # Конфиги isolated-инстансов
```
